...
const form = reactive({ id: '', title: '', date: new Date().toISOString().split('T')[0], color: '#ec4899' });
...
const getDaysInfo = (targetDateStr: string) => {
  ...
  if (diffDays > 0) return { type: 'future', days: diffDays, text: '還有' };
  else if (diffDays < 0) return { type: 'past', days: Math.abs(diffDays), text: '已經' };
  else return { type: 'today', days: 0, text: '就是今天！' };
};
...
const openAddModal = () => {
  ...
  form.id = ''; form.title = ''; form.date = new Date().toISOString().split('T')[0]; form.color = '#ec4899';
  showModal.value = true;
};

// ✨ 改用自訂彈窗
const saveEvent = async () => {
  if (!form.title.trim()) return await customAlert('請輸入名稱喔！', '💡 提示');
  if (!form.date) return await customAlert('請選擇日期喔！', '💡 提示');

  events.value.push({ id: Date.now().toString(), title: form.title, date: form.date, color: form.color });
  showModal.value = false;
};

// ✨ 改用自訂彈窗
const deleteEvent = async (id: string) => {
  if (await customConfirm('確定刪除這個紀念日嗎？', '🗑️ 刪除確認')) {
...
<template>
  <div class="anni-container">
    <div class="toolbar">
      ...
      <button class="add-btn" :disabled="isLocked" @click="openAddModal">＋ 新增日子</button>
    </div>

    <div class="hint-bar locked-hint" v-if="isLocked">🔒 列表已鎖定，請點擊上方按鈕解鎖以新增或刪除</div>
    <div class="hint-bar" v-else>💡 編輯模式已開啟！點擊卡片右上角的 ✕ 即可刪除。</div>
    ...
    <div class="events-grid">
      <div v-for="ev in sortedEvents" :key="ev.id" class="event-card" :style="{ borderTopColor: ev.color }">
        <button v-if="!isLocked" class="del-btn" @click="deleteEvent(ev.id)">✕</button>
        <div class="e-header"><div class="e-title" :style="{ color: ev.color }">{{ ev.title }}</div><div class="e-date">{{ ev.date.replace(/-/g, ' / ') }}</div></div>
    ...
    <div v-if="showModal" class="modal-overlay">
      <div class="modal-card">
        <h3>⏳ 新增日子</h3>
        <div class="form-group"><label>名稱</label><input type="text" v-model="form.title" placeholder="例如：學測、在一起、生日"></div>
        ...
        <div class="modal-actions"><button @click="showModal = false">取消</button><button class="save-btn" @click="saveEvent">確定新增</button></div>
      </div>
    </div>
  </div>
</template>
...
.event-card { background: white; border-radius: 16px; padding: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border-top: 6px solid #ccc; position: relative; transition: transform 0.2s, box-shadow 0.2s; }
.event-card:hover { transform: translateY(-3px); box-shadow: 0 8px 20px rgba(0,0,0,0.08); }
.del-btn { position: absolute; top: 10px; right: 10px; background: #fee2e2; color: #ef4444; border: none; width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; font-weight: bold; transition: 0.2s; }
.del-btn:hover { background: #ef4444; color: white; transform: scale(1.1); }
.e-header { margin-bottom: 15px; } .e-title { font-size: 1.2rem; font-weight: bold; margin-bottom: 4px; padding-right: 25px; } .e-date { font-size: 0.85rem; color: #94a3b8; letter-spacing: 1px; }
.e-body { display: flex; align-items: baseline; gap: 8px; justify-content: center; padding: 10px 0 20px 0; }
.e-prefix { font-size: 1rem; color: #64748b; font-weight: bold; } .e-days { font-size: 3.5rem; font-weight: 900; line-height: 1; text-shadow: 2px 2px 4px rgba(0,0,0,0.05); } .e-suffix { font-size: 1rem; color: #64748b; font-weight: bold; }
...
.form-group { margin-bottom: 15px; } label { display: block; font-size: 0.85rem; color: #64748b; margin-bottom: 8px; font-weight: bold; }
input { width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; box-sizing: border-box; font-size: 1rem; outline: none; } input:focus { border-color: #ec4899; box-shadow: 0 0 0 3px rgba(236, 72, 153, 0.1); }
.color-palette { display: flex; gap: 10px; justify-content: space-between; } .color-swatch { width: 32px; height: 32px; border-radius: 50%; cursor: pointer; transition: 0.1s; border: 2px solid transparent; } .color-swatch.selected { transform: scale(1.15); border-color: #333; box-shadow: 0 2px 8px rgba(0,0,0,0.2); }
.modal-actions { display: flex; gap: 10px; margin-top: 25px; } .modal-actions button { flex: 1; padding: 12px; border: none; border-radius: 8px; cursor: pointer; font-size: 1rem; font-weight: bold; transition: 0.2s; } .save-btn { background: #ec4899; color: white; } .save-btn:hover { background: #db2777; }
</style>